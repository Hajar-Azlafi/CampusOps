...
const emptyForm = { nom: '', code: '', description: '' }

export default function EquipmentFormModal({ open, mode, initialData, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'create') {
        ...
        const updated = await updateEquipment(initialData.id, payload)
        onSuccess(updated)
      }
    } catch (err) {
      if (err.response?.status === 409) {
        setError(err.response.data?.message || 'Ce nom ou ce code est déjà utilisé')
