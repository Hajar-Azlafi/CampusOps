...
import { createProgram, updateProgram } from '../api/programsApi'
import { typeFormationLabel } from '../constants/formation'
import { IconX } from './icons'

const emptyForm = { nom: '', code: '', description: '', departmentId: '', levelId: '' }

export default function ProgramFormModal({ open, mode, initialData, departments, levels = [], onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'create') {
        ...
        const updated = await updateProgram(initialData.id, payload)
        onSuccess(updated)
      }
    } catch (err) {
      if (err.response?.status === 409) {
        setError(err.response.data?.message || 'Ce code est déjà utilisé')
      ...
      <div className="relative bg-surface rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h3 className="font-display text-lg font-semibold text-ink">
            {mode === 'create' ? 'Nouvelle filière' : 'Modifier la filière'}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-ink/80 mb-1.5">Département</label>
            <select
              required
            ...
            >
              ...
              {levels.map((l) => (
                <option key={l.id} value={l.id}>
                  {l.nom} — {typeFormationLabel(l.typeFormation)}
                </option>
              ))}
            </select>
          </div>
